...
RENDER.hexgl.tracks = RENDER.hexgl.tracks || {};

RENDER.hexgl.tracks.Cityscape = {
	...
	{
		this.lib = new RENDER.threejs.Loader(opts);
	...
	buildMaterials: function(quality)
	{
		...
		{
			this.materials.track = RENDER.Utils.createNormalMaterial({
			...
			this.materials.bonusBase = RENDER.Utils.createNormalMaterial({
			...
			this.materials.ship = RENDER.Utils.createNormalMaterial({
			...
			this.materials.scrapers1 = RENDER.Utils.createNormalMaterial({
			...
			this.materials.scrapers2 = RENDER.Utils.createNormalMaterial({
			...
			this.materials.start = RENDER.Utils.createNormalMaterial({
	...
	buildScenes: function(ctx, quality)
	{
		...
		// SHIP CONTROLS
		var shipControls = new RENDER.hexgl.ShipControls(ctx);
		...
		ctx.components.shipEffects = new RENDER.hexgl.ShipEffects(fxParams);
		...
		// CAMERA
		ctx.components.cameraChase = new RENDER.hexgl.CameraChase({
			target: ship,
			camera: camera,
