...
RENDER.hexgl.HexGL = function(opts)
{
	var self = this;
	...
	this.track = RENDER.hexgl.tracks[ opts.track == undefined ? 'Cityscape' : opts.track ];
...
RENDER.hexgl.HexGL.prototype.start = function()
{
...
RENDER.hexgl.HexGL.prototype.reset = function()
{
	...
	RENDER.Audio.volume('wind', 0.35);
	RENDER.Audio.play('bg');
	RENDER.Audio.play('wind');
}

RENDER.hexgl.HexGL.prototype.restart = function()
{
...
RENDER.hexgl.HexGL.prototype.update = function()
{
...
RENDER.hexgl.HexGL.prototype.init = function()
{
	this.initHUD();
...
RENDER.hexgl.HexGL.prototype.load = function(opts)
{
	this.track.load(opts, this.quality);
}

RENDER.hexgl.HexGL.prototype.initGameplay = function()
{
	var self = this;

	this.gameplay = new RENDER.hexgl.Gameplay({
		mode: this.mode,
		hud: this.hud,
...
RENDER.hexgl.HexGL.prototype.displayScore = function(f, l)
{
	this.active = false;

	var tf = RENDER.Timer.msToTimeString(f);
	var tl = [
		RENDER.Timer.msToTimeString(l[0]),
		RENDER.Timer.msToTimeString(l[1]),
		RENDER.Timer.msToTimeString(l[2])
	];
	...
	if(this.gameplay.result == this.gameplay.results.FINISH)
	{
		...
		// ladder record
		var p = RENDER.hexgl.Ladder.global[t][d][RENDER.hexgl.Ladder.global[t][d].length-2];
	...
	sf != undefined && (sf.href='http://www.facebook.com/sharer.php?s=100'
		...
		+'&p[summary]='+encodeURIComponent('HexGL is a futuristic racing game built by Thibaut Despoulain (RENDER) using HTML5, Javascript and WebGL. Come challenge your friends on this fast-paced 3D game!')
		+'&p[url]='+encodeURIComponent('http://hexgl.RENDER.com')
		+'&p[images][0]='+encodeURIComponent('http://hexgl.RENDER.com/image.png'));

	RENDER.hexgl.Ladder.displayLadder('finish-ladder', t, d, 8);
...
RENDER.hexgl.HexGL.prototype.initRenderer = function()
{
	...
	this.manager = new RENDER.threejs.RenderManager(renderer);
}

RENDER.hexgl.HexGL.prototype.initHUD = function()
{
	if(!this.displayHUD) return;
	this.hud = new RENDER.hexgl.HUD({
		width: this.width,
		height: this.height,
...
RENDER.hexgl.HexGL.prototype.initGameComposer = function()
{
	...
	var effectHex = new THREE.ShaderPass( RENDER.threejs.Shaders[ "hexvignette" ] );
...
RENDER.hexgl.HexGL.prototype.createMesh = function(parent, geometry, x, y, z, mat)
{
...
RENDER.hexgl.HexGL.prototype.tweakShipControls = function()
{
